...
void LKTECH_MG_Init(LKTECH_MG_Handle* hmotor, FDCAN_HandleTypeDef* hcan, FDCAN_TxHeaderTypeDef* htxheader, FDCAN_RxHeaderTypeDef* hrxheader, FDCAN_FilterTypeDef* hrxfilter, uint32_t motor_id, float gear_ratio, float kt)
{
  ...
  hmotor->kt = kt;
  hmotor->status = LKTECH_MG_MOTOR_STATUS_UNKNOWN;
  hmotor->lastCANReceivedTimestamp = 0;
	
  hmotor->txHeader = htxheader;
  hmotor->rxHeader = hrxheader;
  hmotor->rxFilter = hrxfilter;
  
  HAL_FDCAN_ConfigFilter(hcan, hmotor->rxFilter); 
...
void LKTECH_MG_GetFeedback(LKTECH_MG_Handle* hmotor, uint8_t rxbuf[])
{
  hmotor->lastCANReceivedTimestamp = HAL_GetTick();
  switch (hmotor->task)
  {
...
//				hmotor->angle.f = ((float)hmotor->encoderRaw.b16) * 360.0f / 16383.0f / hmotor->gearRatio;
      ...
      break;
      case LETECH_MG_CAN_BUS_TASK_ENABLE:
      if (rxbuf[0] == LKTECH_MG_COMMAND_ENABLE)
        hmotor->status = LKTECH_MG_MOTOR_STATUS_ENABLED;
      break;
      case LETECH_MG_CAN_BUS_TASK_DISABLE:
      if (rxbuf[0] == LKTECH_MG_COMMAND_DISABLE)
        hmotor->status = LKTECH_MG_MOTOR_STATUS_DISABLED;
  	default:
  		break;
  }
}
...
{
  hmotor->task = LETECH_MG_CAN_BUS_TASK_DISABLE;
...
{
  hmotor->task = LETECH_MG_CAN_BUS_TASK_ENABLE;
