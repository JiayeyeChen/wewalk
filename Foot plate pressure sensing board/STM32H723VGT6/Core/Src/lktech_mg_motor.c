...
void LKTECH_MG_Init(LKTECH_MG_Handle* hmotor, FDCAN_HandleTypeDef* hcan, uint32_t motor_id, float gear_ratio, float kt)
{
  ...
  //hmotor->txHeader.RTR = 0;
	//hmotor->txHeader.StdId = hmotor->canID;
	hmotor->txHeader.Identifier=hmotor->canID;
  hmotor->txHeader.IdType=FDCAN_STANDARD_ID;
  hmotor->txHeader.TxFrameType=FDCAN_DATA_FRAME;
  hmotor->txHeader.DataLength=8;
  hmotor->txHeader.ErrorStateIndicator=FDCAN_ESI_ACTIVE;            
  hmotor->txHeader.BitRateSwitch=FDCAN_BRS_OFF; 
  hmotor->txHeader.FDFormat=FDCAN_CLASSIC_CAN;
  hmotor->txHeader.TxEventFifoControl=FDCAN_NO_TX_EVENTS;
  hmotor->txHeader.MessageMarker=0;                           
}
...
void LKTECH_MG_SendSingleCommand(LKTECH_MG_Handle* hmotor, uint8_t command, uint8_t task_name)
{
  ...
  HAL_FDCAN_AddMessageToTxFifoQ(hmotor->hcan, &(hmotor->txHeader), hmotor->txBuf);
}

void LKTECH_MG_GetFeedback(LKTECH_MG_Handle* hmotor, FDCAN_RxHeaderTypeDef* rxheader, uint8_t rxbuf[])
{
  switch (hmotor->task)
  {
    ...
    case LKTECH_MG_CAN_BUS_TASK_READ_ACCELERATION:
      ...
      case LETECH_MG_CAN_BUS_TASK_POSITION_CONTROL_3_SINGLE_TURN:
...
{
  hmotor->task = LETECH_MG_CAN_BUS_TASK_READ_ANGLE_SINGLE_TURN;
...
void LETECH_MG_PositionControl1MultiTurn(LKTECH_MG_Handle* hmotor, float pos_multi)
{
  ...
  pos.b32 = (int32_t)(pos_multi * 100.0f * hmotor->gearRatio);
...
void LETECH_MG_PositionControl3SingleTurn(LKTECH_MG_Handle* hmotor, float pos_single, uint8_t dir)
{
  hmotor->positionControlSingleTurnSet.f = pos_single;
  union UInt16UInt8 pos;
  pos.b16 = (uint16_t)(pos_single * 100.0f * hmotor->gearRatio);
  hmotor->task = LETECH_MG_CAN_BUS_TASK_POSITION_CONTROL_3_SINGLE_TURN;
  memset(hmotor->txBuf, 0, 8);
  hmotor->txBuf[0] = LKTECH_MG_COMMAND_POSITION_CONTROL3;
  hmotor->txBuf[1] = dir;
  hmotor->txBuf[4] = pos.b8[0];
  hmotor->txBuf[5] = pos.b8[1];
  HAL_FDCAN_AddMessageToTxFifoQ(hmotor->hcan, &(hmotor->txHeader), hmotor->txBuf);
